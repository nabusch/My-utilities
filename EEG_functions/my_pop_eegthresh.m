% My own copy. Only difference to original: pop up an eegplot everytime,
% not only when called from the GUI as in the original.
%
...
if calldisp
    ...
    if icacomp == 1    
        eegplot( EEG.data(elecrange,:,:), 'srate', EEG.srate, 'limits', [EEG.xmin EEG.xmax]*1000 , 'command', command, eegplotoptions{:}, 'spacing', 45); 
    else
        eegplot( icaacttmp, 'srate', EEG.srate, 'limits', [EEG.xmin EEG.xmax]*1000 , 'command', command, eegplotoptions{:}, 'spacing', 45); 
    end;
else 
    if reject == 1
